...
public class EmpiReS {
    public enum EmpiReTask {
            ECMMAPPER("ecmapper"),
        BUILDINDEX("build_index"),
        EQCFROMBAMS("ec_from_bams"),
        DIFFEXPONEXPRSET("diffexp_on_eset"),
        EQCINPUT("diffexp_diffsplic_on_eccounts"),
        TRESTIMATEINPUT("diffsplic_on_transcript_estimates"),
        GENERATEREADS("generate_reads"),
        SIMULATECOUNTS("simulate_counts"),
        UNKNOWN("unknown")
        ;

        String label;
        EmpiReTask(String label) {
            this.label = label;
            EnumGetter.add(label, this);
        }

        public static EmpiReTask get(String label) {
				if(map(EmpiReTask.values(), (_s) -> _s.label).contains(label))
                return EnumGetter.get(label, true);
            else
                return EmpiReTask.UNKNOWN;
        }
    }
    public static void main(String[] args) {
		  String taskHelp = "Please provide a task as first parameter. The following tasks are available:\n\tbuild_index - build the genome index for the equivalence class variant of contextmap\n\tecmapper - mapping of fastq files to equivalence classes\n\tec_from_bams - derive equivalence class counts from mapped reads (BAM files)\n\tdiffexp_on_eset - analyse differential expression from expression set like formatted data\n\tdiffexp_diffsplic_on_eccounts - analyse differential expression and differential alternative splicing from equivalence class counts derived from ecmapper or ec_from_bams\n\tdiffsplic_on_transcript_estimates - analyse differential alternative splicing from transcript estimate counts generated by SALMON, kalisto or StringTie\n\tsimulate_counts - simulate counts for a benchmarking dataset based on measured data with many replicates\n\tgenerate_reads - generate reads for the simulated counts for a benchmarking dataset\n";
        if(args.length == 0) {
            System.err.printf(taskHelp);
            return;
        }
        boolean help=false;
        for(int i=0; i<args.length; i++){
            if(args[i].equals("-h") || args[i].equals("--help") || args.length==1){
                 help=true;
            }
        }
        String[] subparams = Arrays.copyOfRange(args, 1, args.length);
        switch (EmpiReTask.get(args[0])) {
            case ECMMAPPER:
                if(help){
                    System.err.print("Use the equivalence class variant of contextmap to map reads\n\nThis task needs the following inputs:\n\t-index: binary index build for the given genome/annotation by the build_index task\n\t-table: tsv file containing for each sample that should be mapped one line. It needs to contain at least the columns:\n\t\tlabel: name of the sample\n\t\tcondition: condition of the sample, replicate measurements share the same value in this column\n\t\tfw: path to the forward strand fastq file\n\t\tstrandness:  a value of \"true\" indicates that the first read defines the strandness, while a value of \"false\" indicates that the second read defines the strandness and an empty string indicates that the measurement was strand unspecific.\n\t\toptional: rw: path to the reverse strand fastq file in case of a paired-end sequencing experiment.\n\t-o path to the output count table\n\t optional: -nthreads: number of parallel threads used - default 10\n\toptional: -basedir: path to a directory, if given the paths given in the sample table are relative to this directory\n");
                    return;
                }
                ECMMapper.main(subparams);
                return;
            case BUILDINDEX:
                if(help){
                ...
                BuildECMMapperIndex.main(subparams);
                return;
            case EQCFROMBAMS:
                if(help){
                ...
                DeriveEQCCountsFromBams.main(subparams);
                return;
            ...
            case SIMULATECOUNTS:
                ...
                GenerateSimulatedTrCounts.main(subparams);
                return;
				case TRESTIMATEINPUT:
					if(help){
						System.err.print("Analyses differential alternative splicing for a table of transcript estimate counts, generated by SALMON, kallisto or StringTie\n\nThis task needs the following inputs:\n\t-dir directory containing a subdirectory for each measurements which contains the output of SALMON, kallisto or StringTie\n\t-gtf genome annotation in gtf format\n\t-samples a tsv file containing for each sample one line. Each line should contain the name of the sample and its condition (replicates share the same condition value) in the format <sample name>\t<condition>.\n\t-o path to the output tsv\n\toptional: -cond1 and -cond2 conditions from the sample table for which the analysis should be done. By default the first two conditions from the sample table are used ");
						return;
					}
					TrEstimateInput.main(subparams);
					return; 
				default:
					 System.err.printf(taskHelp);
                return;
